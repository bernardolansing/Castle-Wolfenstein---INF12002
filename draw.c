...
void morte()
{
    DrawRectangle(0, 0, largura, altura, (Color)
    {
        0, 0, 0, 210
    });
    DrawTextEx(fonte.alpha_beta, "Voce morreu!", (Vector2)
    {
        largura / 2, altura / 2
    }, 24, 5, ORANGE);
}
...
void draw_jogador()
{
    ...
    if (GetTime() - player.horadisparo < 0.5)
    {
        ...
        {
        case 'C':
            imgplayer = LoadTexture("resources/player/player-up-shoot.png");
            break;
        case 'B':
            imgplayer = LoadTexture("resources/player/player-down-shoot.png");
            break;
        case 'D':
            imgplayer = LoadTexture("resources/player/player-right-shoot.png");
            break;
        case 'E':
            imgplayer = LoadTexture("resources/player/player-left-shoot.png");
            break;
        }
    }
...
void draw_hud()
{
    ...
    DrawRectangle(hud.x, hud.y, hud.width, hud.height, (Color)
    {
        0, 0, 0, 200
    });
